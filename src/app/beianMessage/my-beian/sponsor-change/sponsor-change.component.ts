...
@Component({
  selector: 'app-sponsor-change',
  templateUrl: './sponsor-change.component.html',
  styleUrls: ['./sponsor-change.component.css']
})
export class SponsorChangeComponent implements OnInit {
  ...
  getSponsorDetail():void {
    ...
    this.myBeianService.sponsorDetail(this.sponsorDetailParams).subscribe(data =>{
      if (data.code === '0'){
        ...
        // 获取返回数据
        this.sponsorDetailData = data.result.sponsorEntity;

        this.inItSelectCity(data.result.sponsorEntity.provinceId);
        this.inItSelectCountry(data.result.sponsorEntity.cityId);
  ...
  nextSubmitFunction():void {
    this.myBeianService.changeSponsorData(this.uploadSponsorParams).subscribe(data => {
      if (data.code === '0'){
        this.orderId = data.result.orderId;
        this.route.navigate([`/home/sponsor/upload`], {queryParams: {sponsorId: this.sponsorId ,orderId: this.orderId, type:this.type}});
      } else {
        this.message.warning(data.message);
      }
    });
  ...
  submitSponsorData():void {
    ...
    if(this.sponsorDetailData.typeId === ''
      ||this.sponsorDetailData.certificationId === ''
    ...
    if (this.sponsorDetailData.certificationNum === '') {
      this.message.warning('主办单位证件号码不能为空！');
      return;
    }
    ...
    if (this.sponsorDetailData.sponsorMailAddress === '') {
      this.message.warning('主办单位通讯地址不能为空！');
      return;
    }
    ...
    if (this.sponsorDetailData.telephone1 === '') {
      this.message.warning('联系方式1不能为空！');
      return;
    }
    if (this.sponsorDetailData.telephone2 === '') {
      this.message.warning('联系方式2不能为空！');
      return;
    }
    ...
    if (!phoneReg.test(this.sponsorDetailData.emergencyPhone)) {
      this.message.warning('请输入正确的紧急联系方式！');
      return;
    }
    if (!phoneReg.test(this.sponsorDetailData.telephone1)) {
      this.message.warning('请输入正确的联系方式1！');
      return;
    }
    if (!emailReg.test(this.sponsorDetailData.email)) {
      this.message.warning('请输入正确格式的邮箱！');
      return;
    }
    if (this.sponsorDetailData.telephone1 == this.sponsorDetailData.emergencyPhone) {
  ...
  ngOnInit() {
    ...
    // 获取主办单位性质下拉列表， 证件类型下拉列表
    // this.showCompanyType();
    // to solve the citys data Not loaded
    // this.getSponsorDetail();
  }
